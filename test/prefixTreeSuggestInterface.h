//
// Created by Benjamin D. Horn on 3/3/17.
//

#ifndef DATASTRUCTS_PREFIXTREESUGGESTINTERFACE_H
#define DATASTRUCTS_PREFIXTREESUGGESTINTERFACE_H
...
class prefixTreeSuggestInterface {
    ...
    void addSnippets(const string& document, prefixTree& p, int num_chars){
    ...
    int run(int argc, char* argv[]){ // use to be main
        ...
        while(new_char != '@'){  // Excape if @ is typed
            ...
            if(start.length() > 2){
                ...
                int f_size = finishes.size();
                for(unsigned int i = 0; i < std::min(30, f_size); i++){
                    // indent to match
...
#endif //DATASTRUCTS_PREFIXTREESUGGESTINTERFACE_H
